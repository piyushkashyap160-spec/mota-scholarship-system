      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between min-h-[5.5rem] py-2.5 gap-4">
          ...
          <div className="flex items-center space-x-3">
            ...
            {/* Judge Demo Quick Switcher Button */}
            <div className="hidden lg:flex items-center pl-2 border-l border-slate-200">
              <button
                onClick={onSwitchDemo}
                className="px-2.5 py-1 text-xs font-medium rounded-full bg-slate-100 hover:bg-slate-200 text-slate-700 border border-slate-300 transition-colors flex items-center space-x-1"
                title="Quickly switch between Admin Scrutiny & Applicant view for demo"
              >
                <span>Demo View:</span>
                <strong className="text-gov-navy underline">
                  {currentUser?.role === 'admin' ? 'Switch to Applicant' : 'Switch to Admin'}
                </strong>
              </button>
            </div>
          </div>
